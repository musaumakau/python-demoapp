...
pipeline {
   ...
   stages {
     stage('Logging into AWS ECR') {
      steps {
         script {
             sh "aws ecr get-login-password - region ${AWS_DEFAULT_REGION} | docker login -u AWS -p $(aws ecr get-login-password --region ${AWS_DEFAULT_REGION)453702019477.dkr.ecr.eu-west-1.amazonaws.com/python-app
       }
